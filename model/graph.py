...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph=False, layer=1, data_path=None, threshold=0.01, tau=1):
        ...
        self.label_hier = torch.load('hier_label.pt')
        self.tokenizer = AutoTokenizer.from_pretrained('bert-base-uncased')
        self.i2label = torch.load('i2label.pt')
        i2label = {xid: self.tokenizer.decode(x) for xid, x in self.label_hier.items()}

        self.dataset = './PDTB/Ji/'
        self.i2top = [x.strip() for x in open(self.dataset + '/data/class4.txt').readlines()]
        self.top2i = dict((x, xid) for xid, x in enumerate(self.i2top))

        self.i2sec = [x.strip() for x in open(self.dataset + '/data/class11.txt').readlines()]
        self.sec2i = dict((x, xid) for xid, x in enumerate(self.i2sec))

        self.i2conn = [x.strip() for x in open(self.dataset + '/data/conn.txt').readlines()]
        self.conn2i = dict((x, xid) for xid, x in enumerate(self.i2conn))

        # self.label_name按照longest进行padding
        self.label_name = self.i2top + self.i2sec + self.i2conn
        ...
        if self.graph:
            ...
            num_class = 0
            for s in self.label_hier:
                for v in self.label_hier[s]:
                    path_dict[v].append(s)
                    if num_class < v:
                        num_class = v
            ...
            if GRAPH == 'GRAPHORMER':
                ...
                # 从节点n到根节点之间的路径，此处res采用二维矩阵，因为可能会有多条路径到达根节点
                def get_root(path_dict, n):
                    res = []
                    length = len(path_dict[n])
                    for i in range(length):
                        ...
                        while node != temp_n:
                            ret.append(node)
                            node, temp_n = path_dict[node][0], node
                        ret.append(-1)
                        res.append(ret)
                    return res

                self.inverse_label_list = {}
                # x = get_root(path_dict, 17)
                for i in range(num_class):
                    self.inverse_label_list.update({i: get_root(path_dict, i)})
                print("self.inverse_label_list:")
                print(self.inverse_label_list)
                ...
                def get_shortest_distance(node1, node2):
                    def get_distance(path1, path2):
                        ...
                        path = []
                        dis = 0
                        while p < len(path1) and q < len(path2):
                            dis += abs(path1[p] - path2[q])
                            if path1[p] > path2[q]:
                                path.append(path1[p])
                                p += 1
                            elif path1[p] < path2[q]:
                                path.append(path2[q])
                                q += 1
                            else:
                                break
                    ...
                    paths1 = self.inverse_label_list[node1]
                    paths2 = self.inverse_label_list[node2]
                    paths1_length = len(paths1)
                    paths2_length = len(paths2)

                    import sys
                    dis = sys.maxsize
                    for i in range(paths1_length):
                        for j in range(paths2_length):
                            temp_dis, temp_path = get_distance(paths1[i], paths2[j])
                            if temp_dis <= dis:
                                dis = temp_dis
                                path = temp_path
                    node_list[(node1, node2)] = path
                    return dis

                self.label_id = torch.arange(self.label_num)
                self.distance_mat = self.label_id.reshape(1, -1).repeat(self.label_num, 1)
                hier_mat_t = self.label_id.reshape(-1, 1).repeat(1, self.label_num)
                self.distance_mat.map_(hier_mat_t, get_shortest_distance)
                ...
                self.id_embedding = nn.Embedding(self.label_num + 1, config.hidden_size, self.label_num)
                self.distance_embedding = nn.Embedding(140, 1, 0)
                self.edge_embedding = nn.Embedding(self.label_num + 1, 1, 0)
    ...
    def forward(self, inputs_embeds, attention_mask, labels, embeddings):
        ...
        if self.graph:
            if GRAPH == 'GRAPHORMER':
                label_emb = label_emb + self.id_embedding(self.label_id[:, None].expand(-1, expand_size)).view(1, -1,
                                                                                                    self.config.hidden_size)
                extra_attn = self.distance_embedding(self.distance_mat) + self.edge_embedding(self.edge_mat).sum(
                    dim=1) / (self.distance_mat.view(-1, 1) + 1e-2)
